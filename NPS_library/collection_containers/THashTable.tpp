...
template <typename T>
THashTable<T>::HashEntry::HashEntry(char *_key, const T &_value)
{
...
template <typename T> void THashTable<T>::put(char *key, const T &value)
{
    _error = false;
    List* list = find_list(key);
    HashEntry* entry;
...
template <typename T> T THashTable<T>::remove(char *key)
{
    List *list = find_list(key);
    if(list)
    {
