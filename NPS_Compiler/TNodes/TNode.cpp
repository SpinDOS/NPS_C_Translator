...
TBranch* NPS_Compiler::GetTBranch(LexemeWord *lexeme, bool &hasLeft, bool &expectedRight)
{
    ...
    switch (lexeme->code / 100)
    {
        case 1:
        case 4:
            if(hasLeft || !expectedRight){ReportError("error", "Missing ;"); return nullptr;}
            hasLeft = true;
            expectedRight = false;
            result->Priority = 100;
            result->NumOfChildren = 0;
            cout << "Const or Variable" << endl;
            break;
        case 2:
            switch (lexeme->code)
            {
                ...
                case 204: // (
                    if(!expectedRight){ReportError("err", "("); return nullptr;}
                    result->Priority = 2;
                    result->NumOfChildren = 0; // ?
                    expectedRight = true;
                    cout << "(" << endl;
                    break;
                case 205: // )
                    if(!hasLeft){ReportError("err", "Expected something before )"); return nullptr;}
                    expectedRight = false;
                    hasLeft = true;
                    result->Priority = 2;
                    result->NumOfChildren = 0; // ?
                    ...
                    break;
                case 206: // [
                    if(!hasLeft){ReportError("err", "Expected array object before ["); return nullptr;}
                    expectedRight = true;
                    hasLeft = false;
                    result->Priority = 2;
                    result->NumOfChildren = 0; // ?
                ...
                case 214: // !
                case 215: // ~
                    if(hasLeft){ReportError("error", "No expected left operand (~!)"); return nullptr;}
                    result->Priority = 3;
                ...
                case 219: // / %
                case 220:
                    if(!hasLeft){ReportError("error", "expected left operand (% /)"); return nullptr;}
                    result->Priority = 5;
                    result->NumOfChildren = 2;
                    hasLeft = false;
                    expectedRight = true;
                ...
                case 233: // !=
                    ...
                    expectedRight = true;
                    hasLeft = false;
                    cout << "Binary logic operation == != " << endl;
                    break;
                case 208: // .
                case 209: // ->
                    if(!hasLeft){ReportError("error", "Expected left operand as object ->"); return nullptr;}
                    result->Priority = 2;
                    result->NumOfChildren = 2;
                    expectedRight = true;
                    hasLeft = false;
